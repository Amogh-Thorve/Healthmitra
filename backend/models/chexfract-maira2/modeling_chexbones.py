...
from .configuration_chexbones import ChexBonesConfig
...
import torch
from torch import nn
from transformers import PretrainedConfig, Dinov2Model, Dinov2Config
...
class Phi3ImageEmbedding(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, wte=None, **kwargs) -> None:
        ...
        if isinstance(config.img_processor, dict) and config.img_processor.get('name', None) == 'dino_vision_model':
            assert 'image_dim_out' in config.img_processor, 'image_dim_out must be provided for CLIPVisionModel'

            img_proc_config = Dinov2Config(**config.img_processor)

            self.img_processor = Dinov2Model(config=img_proc_config)
        ...
        if projection_cls == 'linear':
            ...
            scale = 1
            activation = nn.GELU()
            if config.img_processor.get("use_geglu", True):
                scale = 0.5
                activation = GeGlu()
